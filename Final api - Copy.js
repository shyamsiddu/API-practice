...
app.get('/indiv/:id', function (req, res) {
   connection.query('select * from shyam where Id=?', [req.query.id], function (error, results) {
	  if (error) throw error;
	  res.end(JSON.stringify(results));
	});
});
...
app.delete('/del/:id', function (req, res) {
    ...
    connection.query('DELETE FROM shyam WHERE id=?', [req.body.id], function (error, results) {
       if (error) throw error;
       res.end(JSON.stringify(results));
     });
 });
